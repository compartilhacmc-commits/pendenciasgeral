...
// ===================================
const SHEET_ID = '1gif9V0RcEFfFEFHE5jovRRHgrKudlDaZcC-t5YGEbHg';
const SHEET_NAME = 'PENDÊNCIAS GERAL';
...
let allData = [];
let filteredData = [];
let chartDistritos = null; // ✅ NOVO GRÁFICO
...
function populateFilters() {
    // ✅ NOVO FILTRO DISTRITO
    const distritos = [...new Set(allData.map(item => item['Distrito']))].filter(Boolean).sort();
    ...
    setMultiSelectText('msDistritoText', [], 'Todos');
...
function applyFilters() {
    const distritoSel = getSelectedFromPanel('msDistritoPanel'); // ✅ NOVO FILTRO
    ...
    setMultiSelectText('msDistritoText', distritoSel, 'Todos');
    ...
    filteredData = allData.filter(item => {
        const okDistrito = (distritoSel.length === 0) || distritoSel.includes(item['Distrito'] || ''); // ✅ NOVO
        ...
        return okDistrito && okStatus && okUnidade && okEsp && okPrest;
    });
...
function clearFilters() {
    ['msDistritoPanel','msStatusPanel','msUnidadePanel','msEspecialidadePanel','msPrestadorPanel'].forEach(panelId => {
        const panel = document.getElementById(panelId);
        if (!panel) return;
        panel.querySelectorAll('input[type="checkbox"]').forEach(cb => cb.checked = false);
    });
...
function updateCharts() {
    // ✅ NOVO GRÁFICO: PENDÊNCIAS POR DISTRITO (SOMENTE COM STATUS PREENCHIDO)
    const distritosCount = {};
    filteredData.forEach(item => {
        const status = item['Status'];
        if (status && status.trim() !== '') {
            const distrito = item['Distrito'] || 'Não informado';
    ...
    createHorizontalBarChart('chartDistritos', distritosLabels, distritosValues, '#3b82f6'); // ✅ AZUL
...
function createHorizontalBarChart(canvasId, labels, data, color) {
    ...
    if (canvasId === 'chartDistritos' && chartDistritos) chartDistritos.destroy();
    ...
    if (canvasId === 'chartDistritos') chartDistritos = chart;
